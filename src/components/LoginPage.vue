...
function syncModeFromUrl() {
  ...
  mode.value = code === ADMIN_CODE ? 'admin' : 'choose'
}

function updateModeUrl(nextMode: 'choose' | 'admin') {
  const nextUrl = nextMode === 'admin'
    ? `${window.location.pathname}?code=${ADMIN_CODE}`
...
function initCanvas() {
  ...
  class Sakura {
    ...
    draw() {
      context.beginPath()
      context.arc(this.x, this.y, this.radius, 0, Math.PI * 2)
      context.fillStyle = sakuraColor
      context.shadowBlur = 15
      context.shadowColor = sakuraColor
      context.fill()
      context.shadowBlur = 0
    }
  }
  ...
  function initParticles() {
    ...
    for (let index = 0; index < numNodes; index += 1) particles.push(new TechNode())
    for (let index = 0; index < numSakura; index += 1) sakuras.push(new Sakura())
  }

  function connectNodes() {
    for (let first = 0; first < particles.length; first += 1) {
      for (let second = first; second < particles.length; second += 1) {
        const nodeA = particles[first]
        const nodeB = particles[second]
        if (!nodeA || !nodeB) continue
        const dx = nodeA.x - nodeB.x
        const dy = nodeA.y - nodeB.y
        const distance = Math.sqrt(dx * dx + dy * dy)
        if (distance < 120) {
          context.strokeStyle = `rgba(0, 243, 255, ${(1 - distance / 120) * 0.2})`
          context.lineWidth = 1
          context.beginPath()
